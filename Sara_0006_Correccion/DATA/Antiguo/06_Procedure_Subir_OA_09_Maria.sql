...
-- cambiar el tipo de dato de numero version
-- crear un nuevo estado para la version 11- EsperaCoordinador 

/*DELIMITER ;;
CREATE PROCEDURE PruebaT(IN idVer INTEGER)
BEGIN
	START TRANSACTION;
	UPDATE Version SET Id_Estado = 3 WHERE Id_Version = idVer;
    IF(idVer = 2)THEN
		ROLLBACK;
        ELSE COMMIT;
    END IF;
