...
DELIMITER ;;
CREATE PROCEDURE Registrar_PV(IN ArrayTodo VARCHAR(2000))
BEGIN
	CALL EXECUTE_ARRAY(
		ArrayTodo,
		"~",
...
CREATE PROCEDURE RegistrarVersion(IN ArrayVersion VARCHAR(2000))
BEGIN
	CALL EXECUTE_ARRAY(
		ArrayVersion,
        "~",
   ...
   Bucle : WHILE (TRUE) DO
    ...
    WHERE Id_Centro = @idCentro AND Id_Rol = 2; -- SE PUEDE HACER DESDE SARA_CRUD
    
    CALL RegistarNotificaion(CONCAT("Nuevo Producto Virtual ha evaluar para ET~1~",@funPrin,"~",@idEval,"~",@idVer,""));
END;;
DELIMITER ;

DROP PROCEDURE IF EXISTS CorreccionVersion;
DELIMITER ;;
CREATE PROCEDURE CorreccionVersion(IN ArrayCorrecion VARCHAR(2000))
BEGIN
	CALL EXECUTE_ARRAY(
		ArrayCorrecion,
        "~",
        "CASE @i
            ...
            WHEN 2 THEN SET @urlVersion = @valor;
		END CASE"
    );
    ...
    IF(@oldUrl != @urlVersion  AND @idFuncionario = @idFun)THEN 
        ...
        IF(@oldEstado = 3 OR @oldEstado = 4)THEN
			CASE  @oldEstado
...
DELIMITER ;;
CREATE PROCEDURE AprobarPV(IN ArrayAprobacion VARCHAR(100))
BEGIN
