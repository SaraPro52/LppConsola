...
SELECT Id_Tema,Nom_Tema FROM V_Detalles_Categoria WHERE Nom_Categoria = "Ambiental Category";
...
CREATE  VIEW V_Categoria AS(
	...
	SELECT  Id_Categoria, Nom_Categoria,v1.Id_Tema, v1.Nom_Tema,Tipo_Tema,
    v2.Id_P_Virtual, v2.Nom_P_Virtual,v2.Palabras_Clave,
	v2.Id_Formato, Nom_Formato, 
    Id_Version, Fecha_Publicacion, Num_Version,
	Id_Estado, Nom_Estado, Id_Rol, Nom_Rol,
...
-- ---------------------------------------------------------------------------------
SELECT a.Id_Version, a.Id_Funcionario, Nom_Funcionario,  
...
Alter VIEW V_Area AS 
(
	SELECT Id_Area, Nom_Area,Id_Centro,Nom_Centro
	FROM V_Area_Centro
);

CALL SARA_CRUD("SELECT","v_Area","Id_Area|Nom_Area","");
...
ALTER VIEW Inabilitar_Funcionario AS 
(
...
DROP VIEW categoria_V
CREATE VIEW categoria_V AS(
	SELECT DISTINCT Id_Categoria, Nom_Categoria FROM V_Detalles_Categoria
)

CALL SARA_CRUD("SELECT","V_Categoria","","");
