function Detalles(Producto, dat) {
    ...
    $(document).on('click', '.accordion', function (e) {
        for (e = 0; e < acc.length; e++) {
            acc[e].onclick = function () {
                var s = $(this);
                var cc = s.attr("class").split(' ');
                cargarComentario(cc[1]);
    ...
    function cargarComentario(id) {
     //Peticion que carga comentarios del articulo
        jso[3] = ['Crud_Controller', '[{opcion:3,tabla2:25,tipo:2,elegir:[0,1,2,3,4],delimitador:[{colum:4,operador:0,valor1:""}],id:0,opSelect:6}]'];
        selector[3] = $("#BaseComentario"+id);
        datos[3] = {nombre: "Comentario", worker: true,id:id};
        ajax(3, datos[3]);
    }
