 ...
 -- call ConsultaActualizar(5);
    ...
    BEGIN
        ...
        CREATE TEMPORARY TABLE Prueba (
            ...
            Num_Version FLOAT DEFAULT null,-- Num Version 
        ...
        INSERT INTO Prueba 
        SELECT DISTINCT Id_P_Virtual, Nom_P_Virtual,null AS Id_Version,null AS Num_Version,"00-00-00" AS Fecha_Vigencia,null AS Url_Version,Nom_Formato, Id_Funcionario
        ...
        CALL EXECUTE_ARRAY(
            ...
            ",",
            "UPDATE  Prueba 
			 SET Id_Version =  	   (SELECT Id_Version FROM Version WHERE Id_P_Virtual = @valor AND Id_Estado IN (6,7) ORDER BY Num_Version DESC LIMIT 1),
				 Num_Version = 	   (SELECT Num_Version FROM Version WHERE Id_P_Virtual = @valor AND Id_Estado IN (6,7) ORDER BY Num_Version DESC LIMIT 1),
                 Fecha_Vigencia =  (SELECT Fecha_Vigencia FROM Version WHERE Id_P_Virtual = @valor AND Id_Estado IN (6,7) ORDER BY Num_Version DESC LIMIT 1),
                 Url_Version =     (SELECT Url_Version FROM Version WHERE Id_P_Virtual = @valor AND Id_Estado IN (6,7) ORDER BY Num_Version DESC LIMIT 1)
	...
	BEGIN
        ...
        SELECT COUNT(*) INTO @cont FROM 42_v_productosactualizar WHERE Id_Funcionario = idFuncionario;
		...
		WHILE(@c < @cont)DO
				...
				FROM 42_v_productosactualizar WHERE Id_Funcionario = ",@idFuncionario," AND Id_P_Virtual NOT IN (",@idis,")
...
-- CALL ConsultaVistaActualizar("1~1~1");
 /*
 SET @nombre = "";
 SET @array = "13~2-013-01-016.doc~0~pv2~pv2~1-1~3-0,2-1";
 SET @array = "13~diseño_Media.png~0~sadas~asdsa~1-0,5-1~";
 CALL RegistarActualizacion(@array,@nombre);
 SELECT @nombre;*/
